...
def submit_quickgen_form():
    ...
    # Example of accessing individual values if needed
    ...
    save_ssh_logs(sshlogs)  # Save the logs
    fslogs = auto_generate_fs_logs(fs_exfiltration)  # Generate daily File System Event logs
    save_fs_logs(fslogs)  # Save the logs
    ftplogs = auto_generate_ftp_logs(ftp_massdownload,ftp_massexfiltration) 
    save_ftp_logs(ftplogs)  # Save the logs
...
def generate_fs_logs_quick():
    ...
    logs = auto_generate_fs_logs(0.45)  
