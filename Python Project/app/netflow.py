...
def generate_single_netflowlog(start_timestamp, src_ip, dst_ip, src_port, dst_port, numb_connect, time_period):
    logs = []
    end_timestamp = start_timestamp + timedelta(minutes=time_period)
...
def auto_generate_netflow_logs(syn_flood_dos_chance, syn_flood_ddos_chance):
    ...
    company_external_ip = "198.51.100.1"  # Company’s external IP
    ...
    for _ in range(num_logs):
        ...
        if random.random() < syn_flood_dos_chance:
            ...
            logs.extend(generate_log(dos_start_time, dos_src_ip, company_external_ip, random.randint(1024, 65535), dos_dst_port, dos_numb_logs, 5))
        ...
        if random.random() < syn_flood_ddos_chance:
            ...
            for _ in range(50):  # 50 different external IPs participating in DDoS
                ...
                logs.extend(generate_log(ddos_start_time, ddos_src_ip, company_external_ip, random.randint(1024, 65535), ddos_dst_port, ddos_numb_logs // 50, 5))
